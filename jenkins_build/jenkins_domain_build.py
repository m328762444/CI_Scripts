...
JOB_Pre = sys.argv[1]
DomBuild = []
#SpeciJobs=['GWM_V2MH_Android_CI_Build','GWM_V2MH_Android_CI_BnI']
SpeciJobs = [JOB_Pre + 'Build', JOB_Pre + 'BnI']
PORT = 29418
HOST = "androidhub.harman.com"
p = subprocess.Popen('whoami', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
stdout, stderr = p.communicate()
USER = stdout.replace('\n','')
USER = 'svc_cpmshint'
server=jenkins.Jenkins("http://10.80.105.178:8080","devbuild","devbuild")
				...
				if tmpnode2:
					...
					idlenode = None				    
                        ...
                        if idlenode:
                                ...
                                if len(idlenodedict[idlenode]):
					pass
				else:
					del idlenodedict[idlenode]
				#print idlenodedict[idlenode]
