...
env.deploy_project_dir = '/data/www/Lwebadmin/'
env.deploy_release_dir = 'releases'
env.deploy_current_dir = 'current'
env.deploy_version=time.strftime("%Y%m%d")+"v2"
@runs_once
def input_versionid():
	return prompt("release input project rollback version ID: ", default="")
@task
@runs_once
def tar_source():
	print yellow("Creating source package...")
	with lcd(env.project_dev_source):
		local("tar -czf %s.tar.gz ." % (env.project_tar_source + env
...
def make_symlink():
	...
	with settings(warn_only=True):
		...
		run ("ln -s %s %s " % (env.deploy_full_path, env.deploy_current_dir))
...
def rollback():
	...
	env.deploy_full_path=env.deploy_project_root + env.deploy_current_dir+"/"+versionid
